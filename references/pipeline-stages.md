...
language switch, inspect the current cache key in `harvest/cli.py::_whisper`; the current
...
`harvest/transcribe.py` downloads best audio through yt-dlp, converts it to 16 kHz mono WAV with
ffmpeg, invokes `whisper-cli`, and parses the generated SRT. `HARVEST_WHISPER_CLI` can select the
binary and `HARVEST_WHISPER_MODEL` selects the GGML model path. `--robust` maps to whisper.cpp's
...
inspect `harvest/transcribe.py` when troubleshooting the actual machine.

## Frames and phash

Unless `--no-vision` is supplied, harvest downloads a video-only stream, samples frames periodically
...
`harvest/vision.py` calls the LM Studio OpenAI-compatible endpoint. Before captioning, it fingerprints
...
isolated `.ocr-venv` through `scripts/ocr_worker.py`; RapidOCR/OpenCV do not enter harvest's main
...
When an OCR track exists, `harvest/fuse.py` compares it with the picked transcript and adds
...
`harvest/danmaku.py` uses fixed approximately 15-second content-time windows. Ordinary duplicate
...
`harvest mcp` exposes the verified operations to an Agent over stdio. It starts asynchronous ingest
